...
@db.execute("CREATE TABLE templates(id INTEGER PRIMARY KEY AUTOINCREMENT, uid integer, name varchar(256), memory text, disk_size text, cpu text, os text, host_name text, machine text, kernel_path text, initrd text, kernel_args text, network_name text, created_at text, FOREIGN KEY(uid) REFERENCES users(id));")  
